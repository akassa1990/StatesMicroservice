...
public class StateMicroserviceTest {
  ...
  public void testGetStatesByName() throws Exception {
    ...
    RequestBuilder requestBuilder =
        MockMvcRequestBuilders.get("http://localhost:8080/api/states/Michigan");
    mockMvc.perform(requestBuilder).andExpect(status().isOk()).andExpect(jsonPath("id", is(22)))
        .andExpect(jsonPath("country", is("USA"))).andExpect(jsonPath("name", is("Michigan")))
        .andExpect(jsonPath("abbr", is("MI"))).andExpect(jsonPath("area", is("250487SKM")))
  ...
  public void testGetAlabamaAndGeorgia() throws Exception {
    ...
    RequestBuilder requestBuilder =
        MockMvcRequestBuilders.get("http://localhost:8080/api/states/ag");
